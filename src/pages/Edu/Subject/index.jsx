...
class Subject extends Component {
  ...
  handleUpdate = async () => {
    ...
    //2.如果用户输入的和原来的相同，则不执行后面的操作
    // 思路；点击更新按钮的时候，把旧的课程分类存起来，点确认的时候，用新数据和老数据比较
    if (this.oldSubjectTitle === subjectTitle) {
      message.error('课程分类名称不能和之前的相同')
    }

    //在异步操作之前加await 就可以让异步执行完毕之后哦，再执行后面的代码
    await this.props.updateSubject(subjectTitle, subjectId)
    message.success('更改成功')

    //手动调用取消按钮的事件函数，让表格行展示内容
    this.handleCancle()
  }
    ...
    confirm({
      title: (
        <>
          <div>
            你确定要删除
            <span style={{ color: 'green', fontSize: 20 }}>{value.title}</span>嘛？
          </div>
        </>
      ),
      ...
      onOk: async () => {
        ...
        //重新请求获取最新的数据
        //如果当前是最后一页，并且最后一页只有一个数据，也不是第一页，那么请求新的数据时候，应该请求上一页的数据
